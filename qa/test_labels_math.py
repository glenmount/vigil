...
def test_doubles_day_sum():
    r = [
        {"staff_id":"D","shift_start":"2025-02-01T

cat > qa/test_labels_math.py <<'PY'
from engine import labels as L
...
def test_doubles_day_sum():
    r = [
        ...
        {"staff_id":"D","shift_start":"2025-02-01T13:00","shift_end":"2025-02-01T23:00"}, # 15h same day
        {"staff_id":"E","shift_start":"2025-02-01T07:00","shift_end":"2025-02-01T14:00"}  # 7h only
    ]
    d = L.doubles_10d(r)
    assert d["D"] == 1
    assert "E" not in d
...
def test_bells_stats():
    ...
    iphr = L.interruptions_per_hr(bells)  # 4 bells over 3h span -> 1.33…
    assert 1.3 < iphr < 1.4
